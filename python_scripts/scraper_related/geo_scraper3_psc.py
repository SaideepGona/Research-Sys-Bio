...
scratch_path_part = "/local/"
scratch_path = scratch_path_part + job_id +"/"
# scratch_path = ""
# subprocess.run(["mkdir", "-p", scratch_path_part])
subprocess.run(["mkdir", "-p", scratch_path+"/tmp/"])
pylon_path = "/pylon5/ms4s84p/saideep/"
num_cpus = multiprocessing.cpu_count()
destination = "encode_scraper/"
...
class Experiment:
    ...
    def process_experiment(self):
        ...
        if len(experiment.controls) == 0:
            ...
            if self.file_state == "fastq":
                ...
                if merged_noncontrols == None:
                    return
                self.create_macs_peak_no_controls(merged_noncontrols)
    ...
    def create_merged_bam(self, is_control):
        ...
        for rep in current_reps:
            ...
            if bam_file == None:
                subprocess.run(["touch", scratch_path + self.accession_id + ".skipped"])
                print("SKIPPED! Something was fishy about the download")
                sys.stdout.flush()
                return
            rep_bam_files.append(bam_file)              # List of (accession + .bam) filenames to be merged
    ...
    def create_macs_peak(self, merged_noncontrols, merged_controls):
        ...
        subprocess.run(["macs2", "callpeak", "--buffer-size", "1000", "--tempdir", scratch_path + "/tmp/", "-t", merged_noncontrols, "-c", merged_controls, "-n", self.accession_id, "--outdir", scratch_path]) # Call Macs Peak of the merged files
        subprocess.run(["cp", scratch_path + self.accession_id + "_summits.bed", pylon_path+destination])
        subprocess.run(["cp", scratch_path + self.accession_id + "_model.r", pylon_path+destination])
        subprocess.run(["cp", scratch_path + self.accession_id + "_peaks.xls", pylon_path+destination])
        subprocess.run(["cp", scratch_path + self.accession_id + "_peaks.narrowPeak", pylon_path+destination])

        delete_file(merged_noncontrols)
        delete_file(merged_controls)

    def create_macs_peak_no_controls(self, merged_noncontrols):
        #print(glob.glob(scratch_path + "*"))
        print("DIRECTLY PRIOR TO PEAK CALLING FILE STATE IS", self.file_state)
        print(merged_noncontrols)
        sys.stdout.flush()

        subprocess.run(["macs2", "callpeak", "--buffer-size", "1000", "--tempdir", scratch_path + "/tmp/", "-t", merged_noncontrols, "-n", self.accession_id, "--outdir", scratch_path]) # Call Macs Peak of the merged files
        subprocess.run(["cp", scratch_path + self.accession_id + "_summits.bed", pylon_path+destination])
        subprocess.run(["cp", scratch_path + self.accession_id + "_model.r", pylon_path+destination])
        subprocess.run(["cp", scratch_path + self.accession_id + "_peaks.xls", pylon_path+destination])
        subprocess.run(["cp", scratch_path + self.accession_id + "_peaks.narrowPeak", pylon_path+destination])
...
class Replicate:
    ...
    def download_to_bam(self):
        ...
        alignment_process = subprocess.Popen(["bowtie2","-p", str(num_cpus), "-x", reference_genome, "-U", download_file], bufsize = 500*10**6, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
        ...
        # error_text = alignment_process.stderr.read()
        ...
        # p_error = error_text.split('\n')
        # pre_percent = ""
        # for x in p_error:
        #     if x == "%":
        #         break
        #     pre_percent += x
        # print(pre_percent, "pre-percent")
        # if float(pre_percent) < 20.0:
        #     return None
...
ref_download_filename = "hg38.chromFa.tar"
